  ...
  for (int iter = 0; iter < max_iters; iter++) {
    ...
    auto worker = [&](int from, int to) {
      ...
      for (int i = from; i < to; i += step) {
        if (i + 1 < p) {
          bool changed = InPlaceMerge(blocks[i], blocks[i + 1], buffer);
          if (changed) {
    ...
    int chunk = std::max(1, (p - start) / (num_threads * 2));

    for (int t = 0; t < num_threads; ++t) {
      int t_start = start + t * chunk * step;
      int t_end = (t == num_threads - 1) ? p : t_start + chunk * step;
      if (t_start >= p) break;
